...
@Singleton
class TaskList5 @Inject()(protected val dbConfigProvider:DatabaseConfigProvider, cc: MessagesControllerComponents)(implicit assetsFinder: AssetsFinder, ec:ExecutionContext)
    ...
    loginForm.bindFromRequest().fold(
      ...
      ld => {
        ...
        model.createUser(username, password).map { _ =>
          Redirect(routes.TaskList5.login).flashing("error" -> "User creation Done.")
        }.recover {
          case _ =>
            Redirect(routes.TaskList5.login).flashing("error" -> "User creation failed.")
        }
      }
    )
  }
